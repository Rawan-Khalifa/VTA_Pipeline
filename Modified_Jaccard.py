...
def jaccard_index_over_roi(mask1, roi_mask):
    """Calculate the Jaccard Index as the intersection over the ROI (mask2)."""
    # Restrict the calculation to the region covered by the ROI
    roi = roi_mask > 0
    
    # Calculate the intersection within the ROI
    intersection = np.logical_and(mask1, roi_mask)[roi].sum()
    
    # Calculate the sum of the ROI mask
    roi_sum = roi.sum()
    
    # Jaccard index as intersection over the ROI
    jaccard = intersection / roi_sum
    return jaccard
...
def process_hemisphere(stim_file_path, base_dir, hemisphere):
    ...
    # Calculate Jaccard Index over the ROI
    jaccard_stn = jaccard_index_over_roi(stim_mask, stn_mask)
    jaccard_gpi = jaccard_index_over_roi(stim_mask, gpi_mask)
